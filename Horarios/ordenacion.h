...
void ordenar_cursos()
{
    ...
    {
        memoria_matriz_int(&aux,2,info_trim[2][i]); //le damos a aux el tamaño para guardar los cursos del trimestre i-esimo
        k=j; //respaldamos el indice del primer elemento del trimestre i-esimo en la matriz C
        l=0;
        while(j<n_cursos && l<info_trim[2][i]) //copiamos en aux los cursos y sus horas del trimestre i-esimo desde la matriz C
        {
            aux[0][l]=C[0][j];
            aux[1][l]=C[1][j];
            j++;
        ...
        shell_cursos(aux,info_trim[2][i]); //ordena los cursos del trimestre i-esimo
        l=0;
        //if(k!=0)
            //aux[0][l]=-aux[0][l];
        while(k<n_cursos && l<info_trim[2][i]) //copiamos en C los cursos y sus horas del trimestre i-esimo, ordenanos, desde aux
        {
            C[0][k]=aux[0][l];
            C[1][k]=aux[1][l];
            k++;
